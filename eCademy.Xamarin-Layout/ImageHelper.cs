...
namespace eCademy.Xamarin_Layout
{
    public class ImageHelper
    {
        public static int[] StaticImages = new int[] {
            Resource.Drawable.nine_way_and_gate,
            Resource.Drawable.acroyoga,
            Resource.Drawable.darth_vlad,
            Resource.Drawable.iPhone,
        ...
        public static EventHandler<AdapterView.ItemClickEventArgs> StartImageActivityFromClick(Context context)
        {
            return (sender, args) =>
            {
                var id = StaticImages[args.Position];
                var intent = new Intent(context, typeof(ImageActivity));
                intent.PutExtra("image_resourceId", id);
